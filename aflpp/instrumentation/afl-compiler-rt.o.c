...
void __sanitizer_cov_trace_pc_guard_init(uint32_t *start, uint32_t *stop) {
  ...
  /* Skip the initial bitmap slots reserved for special purposes and start
     assigning guards from AFL_BITMAP_FIRST_LOC onwards. SWAM mode uses
     5 here, upstream AFL++ uses 4. */
  if (__afl_final_loc < AFL_BITMAP_FIRST_LOC) {

    __afl_final_loc = AFL_BITMAP_FIRST_LOC;
