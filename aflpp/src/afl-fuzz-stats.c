  ...
  while ((lptr = fgets(buf, MAX_LINE, f))) {
    ...
    if (*lptr == ':') {
      ...
      *lptr = 0;
      ...
      /* In SWAM mode, do not restore/track cmplog_time separately. The field
         is kept for upstream compatibility, so we ignore cmplog_time in
         fuzzer_stats. */
...
void write_stats_file(afl_state_t *afl, u32 t_bytes, double bitmap_cvg,
  ...
  /* In SWAM mode, overhead includes calibration/sync/trim only and ignores
     cmplog_time. Default mode preserves upstream behavior. */
#ifdef AFL_SWAM
  u64 overhead_ms =
...
/* In SWAM mode, cmplog_time is not tracked separately, but this function is
   kept for upstream API compatibility. Default mode preserves upstream
   behavior. */
