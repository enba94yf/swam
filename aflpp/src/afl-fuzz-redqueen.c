...
u8 input_to_state_stage(afl_state_t *afl, u8 *orig_buf, u8 *buf, u32 len) {
  ...
  /* In SWAM mode, do not accumulate cmplog stage time into the separate
     cmplog_time counter. Keep only calibration/sync/trim aggregated time.
     Default mode preserves upstream behavior. */
#ifndef AFL_SWAM
  update_cmplog_time(afl, &cmplog_start_us);
#endif
