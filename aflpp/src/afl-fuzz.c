...
int main(int argc, char **argv_orig, char **envp) {
  ...
  if (afl->afl_env.afl_custom_mutator_only) {
    ...
    if (!afl->custom_mutators_count) {
      ...
      /* In SWAM mode, a custom mutator must be present when running in
         custom-only mode. */
      FATAL(
          "AFL_SWAM: AFL_CUSTOM_MUTATOR_ONLY is set but no custom mutator "
          "was loaded.\n"
          "Please set AFL_CUSTOM_MUTATOR_LIBRARY to a valid mutator library.");
...
#ifdef AFL_SWAM
  /* Defensive check: when compiled with AFL_SWAM we only support
     "custom mutator only" mode. Any other configuration is rejected
     early to avoid silently running with unsupported heuristics. */
  if (!afl->custom_only) {

    FATAL(
        "This AFL++ SWAM build only supports custom mutator only mode.\n"
        "Please run with AFL_CUSTOM_MUTATOR_LIBRARY=<path> and "
