...
u8 skip_deterministic_stage(afl_state_t *afl, u8 *orig_buf, u8 *out_buf,
...
#ifdef AFL_SWAM
    /* SWAM allocates skip_eff_map / done_inf_map as bytes (not bit-packed),
       to match the baseline implementation more closely. */
  ...
  do {
    ...
    for (; afl->stage_cur < afl->stage_max && afl->stage_cur < len;
      ...
      /* Mark this byte as already inferred. SWAM uses byte maps; upstream uses
         a bitmap. */
#ifdef AFL_SWAM
...
cleanup_skipdet:
  ...
  if (fuzz_nearby) {
    ...
    for (i = 0; i < len; i++) {
      ...
      if (nearby_bytes[i] && !non_eff_bytes[i]) {
#ifdef AFL_SWAM
        skip_eff_map[i] = 1;
#else
        bitmap_set(skip_eff_map, i);
#endif
      }
