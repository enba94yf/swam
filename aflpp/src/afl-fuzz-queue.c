...
#ifdef AFL_SWAM
/* SWAM-style queue entry weighting based on exec time, bitmap size and
   top-rated refcount. */
static double compute_weight(afl_state_t *afl, struct queue_entry *q,
  ...
  /* Execution time term: only apply if we have sane averages and per-entry
     timings. This avoids division-by-zero and NaN weights when timings are
     not initialized yet. */
  if (likely(afl->schedule < RARE) && avg_exec_us > 0.0 &&
      q->exec_us > 0.0) {
  ...
  /* Bitmap size term: operate in log-space but guard against empty bitmaps
     and zero averages. If all bitmaps are tiny, this factor is skipped. */
  if (avg_bitmap_size > 0.0 && q->bitmap_size > 0) {

    double q_log = log((double)q->bitmap_size);
    weight *= (q_log / avg_bitmap_size);

  }

  /* Top-rated refcount term: only enable once we have non-zero average.
     This keeps early runs (where tc_ref is 0 for all entries) stable. */
  if (avg_top_size > 0.0) {

    weight *= (1.0 + ((double)q->tc_ref / avg_top_size));
...
void create_alias_table(afl_state_t *afl) {
  ...
  if (likely(afl->schedule < RARE)) {
    ...
    for (i = 0; i < n; i++) {
      ...
      if (likely(!q->disabled)) {
        ...
        if (q->bitmap_size > 0) {
          ...
          /* Guard against log(0). Very small maps are treated as neutral
             here and will cause the bitmap term to be skipped below. */
          avg_bitmap_size += log((double)q->bitmap_size);
    ...
    if (active > 0) {
      ...
      avg_exec_us /= active;
      if (avg_exec_us <= 0.0) { avg_exec_us = 1.0; }

      /* avg_bitmap_size is an average over log(bitmap_size). It can be 0 if
         all bitmaps are extremely small; we handle that inside compute_weight
         by skipping the bitmap term when the average is non-positive. */
      ...
      /* No active entries (all disabled) – fall back to uniform weights to
         avoid building a degenerate alias table. */
      for (i = 0; i < n; i++) {
...
#ifdef AFL_SWAM
  /* Mark SWAM custom / initial corpus entries. */
  if (afl->current_custom_fuzz || afl->start_time == 0) { q->custom = 1; }
#endif
