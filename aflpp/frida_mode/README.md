...
This artifact does not include the standalone debugging guide that ships with
the upstream AFL++ source tree.

For troubleshooting, prefer to reproduce with a minimal target and enable
verbose logging for FRIDA mode, then iteratively narrow down instrumentation
settings (for example `AFL_FRIDA_INST_RANGES`).
